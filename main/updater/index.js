 ...
 *     '자동'을 켜면 백그라운드에서 주기적으로 조용히 확인/다운로드하고, 창을 닫는 순간 설치까지
 *     조용히 끝낸다. 두 모드 다 "다운로드"만큼은 확인이 시작되면(수동 클릭이든 자동 주기든)
 *     자동으로 진행된다(autoDownload=true 고정) — 확인과 다운로드 사이에 또 버튼을 눌러야 하면
 *     번거로우니까. 모드가 가르는 건 "확인이 언제 시작되는가"(자동 주기적 vs 수동 클릭)와
 *     "다운로드 완료 후 설치가 조용히 되는가, 재시작 확인을 받는가" 두 가지뿐이다.
...
function initUpdater(app, ipcMain, mainWindow, settings) {
  ...
  // 창을 닫아서(트레이로 내려가서) 안 보이게 되는 바로 그 시점 — 자동 모드에서만, 마침 설치할
  // 업데이트가 준비돼 있으면 숨기는 대신 조용히 재시작+설치한다. 수동 모드는 설치 시점도
  // 사용자가 재시작 확인 팝업(update-overlay.js)으로 직접 결정하길 원해서 만든 모드라 여기서
  // 조용히 설치해버리면 그 의도와 어긋난다. main.js의 close 핸들러(숨기기)보다 나중에 등록돼서
  // 나중에 실행되므로, 여기서 종료로 확정하면 숨기기 대신 실제로 꺼지고 isForceRunAfter=true라
  // 설치 후 자동으로 다시 켜진다.
  if (mainWindow) {
    mainWindow.on('close', () => {
