...
| # | 내용 | 스키마 | 상태 |
|---|---|---|---|
| **S1** ✅ | **엔진 코어**: `search_index`를 FTS5 → 일반 테이블(LIKE 스캔) + `chosung` 컬럼으로 재구축(마이그레이션 v3). `search.repository.query()` = 정규화(공백/꼬리말) → 랭킹(제목정확>시작>포함>초성>본문) + `matchedIn`(일치이유). 한글 부분일치("길동"→"홍길동")·띄어쓰기무관("홍 길동"=="홍길동")·초성("ㅎㄱㄷ") 지원. `chosung()`는 SQLite 커스텀 함수로 트리거에서도 사용. `links.searchCandidates`/`discoverRelated 비슷한내용`도 같은 엔진 사용(FTS MATCH 제거). `#/search`에 "제목/초성/본문 일치" 배지. — v2.58.33 | **마이그레이션 v3** | 완료 |
...
## 디자인 시스템 개편 Phase 2 — 라이브 미리보기 (v2.66.2)
- **설정 > 화면 상단 "현재 스타일"** — 앱의 실제 클래스·`var(--*)` 토큰을 그대로 쓴 미니 잇다 UI
  (사이드바+나브 + 카드+할일3줄+입력+버튼 + 미니 캘린더). 테마/강조색/세부디자인을 바꾸면
  토큰이 바뀌어 **JS 없이 자동 반영**. 아래 `#theme-liveLabel`은 "Pure · 블루 · Standard" 식 텍스트.
  `refreshLiveLabel()`이 테마카드·강조색 클릭 시 갱신.
- **테마 카드 미리보기 강화**: 사이드바 조각 + 카드 안에 제목·체크박스 달린 할일 2줄(2번째는 강조색).
  팔레트는 테마별 하드코딩(각 카드가 다른 테마라 live 토큰 못 씀).
...
- **연결/관련 항목 행 클릭 → 화면 이동 대신 그 자리에서 아코디언 미리보기** (`links-ui.js`).
  ...
  유형·날짜·카테고리 + 본문 ~400자 + `[원본으로 이동 →]`. 캐럿 아이콘 회전, 한 번에 하나만 펼침,
  `render()` 후 `reopenPreview()`로 복원. 행 오른쪽 `↗`(`.link-item-go`) = 미리보기 없이 바로 이동.
- **대시보드 ←/→**: 사이드 패널이 열려 있으면 사이드 캘린더(`stepCal`), 닫혀 있으면 상단 날짜(`stepDate`).

## 연결 위젯 개선 — 검색 피커 + 팝오버 넓힘 + 관련항목 2줄 (v2.65.6)
(위 항목이 이걸 이어감)
...
- **설정 수치 컨트롤 통일**: 화면 배율에만 있던 `−/+ · 슬라이더 · 직접입력(number)` 3종 세트를
  사이드바 너비(step 4)·위젯 투명도(step 5)에도 적용. 범위 clamp + 슬라이더/입력 양방향 동기화.
- **대시보드 인사말 글자색**: 설정 > 화면에 `<input type=color>` + 기본값(app_settings: `dash_greeting_color`, 빈 값=기본).
  dashboard.js가 마운트 시 `.dash-greeting h1,p`에 인라인 color로 적용(배경 이미지/다크 대시테마 위에서도 보이게).
