...
export async function mount(root) {
  ...
  async function initBackgroundAndTheme() {
    ...
    try {
      let sp = await s('dashboard_style_preset');
      if (DASH_STYLE_MIGRATE[sp]) {
        sp = DASH_STYLE_MIGRATE[sp];
        window.itda.settings.set({ key: 'dashboard_style_preset', value: sp }).catch(() => {});
      }
      if (DASHBOARD_STYLE_PRESETS.some((p) => p.id === sp && p.id !== 'standard')) layout.dataset.dashstyle = sp;
