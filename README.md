...
- **패키징된 빌드**에서는 앱 시작 몇 초 후 자동으로 새 버전을 확인하고, 있으면
  백그라운드에서 바로 다운로드합니다(클릭 불필요). 다운로드가 끝나도 일하는 도중에
  바로 재시작시키지는 않고, 사용자가 창을 닫는(트레이로 내려가는) 바로 그 순간에
  조용히 설치하고 자동으로 다시 켭니다 — 이 앱은 트레이 상주형이라 "완전히 종료"를
  거의 안 누르는데, mainWindow의 close 이벤트를 직접 감지해서 그 타이밍에 설치하므로
  실제로 적용될 기회를 놓치지 않습니다. 설정(`#/settings`) 화면의 "업데이트" 탭에서
  지금 바로 확인하거나("업데이트 확인"), 다운로드가 끝났는데 바로 적용하고 싶으면
  "지금 재시작해서 설치"를 누를 수도 있습니다(선택 사항).
