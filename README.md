...
- **패키징된 빌드**에서는 앱 시작 몇 초 후 자동으로 새 버전을 확인하고, 있으면
  백그라운드에서 바로 다운로드합니다(클릭 불필요). 다운로드가 끝나면 바로 재시작시키지는
  않고, 앱이 다음에 자연스럽게 종료될 때(트레이 "완전히 종료", OS 종료/로그아웃 등)
  조용히 설치됩니다 — 그래서 사용자가 일하는 도중 갑자기 재시작되는 일 없이, 다음에
  앱을 열면 이미 최신 버전인 것처럼 느껴집니다. 설정(`#/settings`) 화면 하단 "업데이트"
  패널에서 지금 바로 확인하거나("업데이트 확인"), 다운로드가 끝났는데 바로 적용하고
  싶으면 "지금 재시작해서 설치"를 누를 수도 있습니다(선택 사항).
